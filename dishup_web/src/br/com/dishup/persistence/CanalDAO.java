...
public class CanalDAO {
	...
	public void insert(Connection connection, CanalVO canalVO) throws CanalAlreadyExistException, TableFieldCheckException, TableFieldNullValueException, DatabaseException{
		...
		try{
			...
			if(e.getSQLState().equals(SQLSTATE_CODE_23505))
				throw new CanalAlreadyExistException("Canal ja existe no sistema: "+canalVO.toString());
			else if(e.getSQLState().equals(SQLSTATE_CODE_23514))
				throw new TableFieldCheckException("Violacao (CHECK) em algum campo: "+canalVO.toString());
			else if(e.getSQLState().equals(SQLSTATE_CODE_23502))
				throw new TableFieldNullValueException("Campos nao podem ser nulos: "+canalVO.toString());
			else
				throw new DatabaseException("TABLE: canal SQLCODE: "+e.getErrorCode()+" SQLSTATE: "+e.getSQLState()+" SQLMESSAGE:"+e.getMessage());
		}
	}
	
	public CanalVO selectById(Connection connection, int id) throws DatabaseException, CanalNotFoundException{
		...
		try{
			...
			if(rs.next())
				canalVO = new CanalVO(rs.getInt(1), rs.getString(2), rs.getString(3));
			else
				throw new CanalNotFoundException("Canal com o ID: "+id+" nao encontrado no sistema.");
		}catch(SQLException e){
			throw new DatabaseException("TABLE: canal SQLCODE: "+e.getErrorCode()+" SQLSTATE: "+e.getSQLState()+" SQLMESSAGE:"+e.getMessage());
		}
		return canalVO;
	...
	public ArrayList<CanalVO> selectAllOrderById(Connection connection) throws DatabaseException, EmptyTableException{
		...
		try{
			...
			rs = stmt.executeQuery();
			while(rs.next())
				list.add(new CanalVO(rs.getInt(1), rs.getString(2), rs.getString(3)));
			if(list.isEmpty())
